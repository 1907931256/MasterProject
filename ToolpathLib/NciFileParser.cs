...
namespace ToolpathLib
{
    ...
    class NciFileParser
    {
        ...
        internal ToolPath ParsePath(List<string> file)
        {
            ...
            for (int i = 0; i < length; i+=2)
            {
                if (file[i] != "")
                {
                    ...
                    switch (gBlock)
                    {
                        case 0: toolpath.Add(parseLine(BlockType.Rapid, paramArr));
                            break;
                        case 1: toolpath.Add(parseLine(BlockType.Linear, paramArr)); //linear move
                            break;
                        case 2: toolpath.Add(parseArc(BlockType.CWArc, paramArr));//cw arc
                            break;
                        case 3: toolpath.Add(parseArc(BlockType.CCWArc, paramArr));//ccw arc move
                            break;
                        case 4: toolpath.Add(parseDelay(BlockType.Delay, paramArr));//delay dwell
                            break;
                        case 11: toolpath.Add(parseFiveAxis(BlockType.FiveAxis, paramArr));//5axis move
                            break;
                        case 1000:
                        case 1001:
        ...
        /// <returns></returns>
        private PathEntity parseLine(BlockType type, string[] ParamArr)
        { 
        ...
        private PathEntity parseDelay(BlockType type, string[] ParamArr)
        {
        ...
        private PathEntity parseArc(BlockType type, string[] ParamArr)
        {
        ...
        private PathEntity parseFiveAxis(BlockType type, string[] paramArr)
        {
            LinePathEntity entity = new LinePathEntity(type);
            entity.Type = type;
