...
namespace SurfaceModel
{
     ...
     public class Surface2D<T> where T : SurfacePoint, new() 
    {
        protected T[,] _values;
        ...
        private void fillSurfaceArray(double initialZ)
        {
            try
            {
        ...
        public Surface2D(BoundingBox _boundingBoxIn, double initValue, double meshSizeIn)
        {
            Type = SurfaceType.Array2D;
            _meshSize = meshSizeIn;
            _boundingBox = _boundingBoxIn;
            calcSize();
            fillSurfaceArray(initValue);
        }
    }
}
